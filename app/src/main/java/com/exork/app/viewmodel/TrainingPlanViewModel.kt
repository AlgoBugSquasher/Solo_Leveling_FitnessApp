...
import com.exork.app.model.WeeklyBonusEntity
    ...
    fun toggleExerciseCompletion(exercise: PlannedExercise) {
        viewModelScope.launch {
            ...
            if (isNowCompleting) {
                ...
                viewModelScope.launch {
                    if (repository.getEventCountByType(com.exork.app.model.JourneyEventType.TRAINING) == 0) {
                        repository.recordJourneyEvent(
                            eventType = com.exork.app.model.JourneyEventType.TRAINING,
                            title = "FIRST PLAN COMPLETED",
                            description = "Successfully finished a planned training item.",
                            icon = "📋",
                            rarity = com.exork.app.model.JourneyRarity.COMMON
                        )
                    }
                }
